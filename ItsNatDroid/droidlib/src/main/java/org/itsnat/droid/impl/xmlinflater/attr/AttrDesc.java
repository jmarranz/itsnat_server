...
public abstract class AttrDesc
{
    ...
    public static int getIdentifier(String attrValue, Context ctx,XMLLayoutInflateService layoutService)
    {
        return getIdentifier(attrValue, ctx,layoutService,true);
    }

    public static int getIdentifier(String value, Context ctx,XMLLayoutInflateService layoutService,boolean throwErr)
    {
    ...
    public int getIdentifierAddIfNecessary(String value, Context ctx)
    {
        ...
        if (value.startsWith("@+id/") || value.startsWith("@id/")) // Si fuera el caso de "@+mypackage:id/name" ese caso no lo soportamos, no lo he visto nunca aunque en teoría está sintácticamente permitido
        {
            ...
            if (id <= 0)
            {
                ...
                if (value.startsWith("@+id/")) id = inflateService.findViewIdAddIfNecessary(idName);
                else id = inflateService.findViewId(idName);
