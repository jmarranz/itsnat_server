 ...
 *
 * La versión ASV v3 y Renesis v1.1.1 NO soportan la creación/despacho
 * de eventos manual pero lo simulamos de la misma forma que el NetFront.
 *
 * @author jmarranz
 */
public abstract class JSRenderW3CASVRenesisEventImpl extends JSRenderW3CEventImpl
{
    public JSRenderW3CASVRenesisEventImpl()
    {
    }

    public static JSRenderW3CASVRenesisEventImpl getJSRenderW3CASVRenesisEvent(ItsNatDOMStdEvent event)
    {
        if (event instanceof MouseEvent)
            return JSRenderW3CASVRenesisMouseEventImpl.SINGLETON;
        else if (event instanceof ItsNatKeyEvent)
            return JSRenderW3CASVRenesisKeyEventImpl.SINGLETON;
        else if (event instanceof UIEvent)
            return JSRenderW3CASVRenesisUIEventGenericImpl.SINGLETON;
        else if (event instanceof HTMLEventInternal)
            return JSRenderW3CASVRenesisEventDefaultImpl.SINGLETON;
        else if (event instanceof MutationEvent)
            return JSRenderW3CASVRenesisMutationEventImpl.SINGLETON;
        else
            return JSRenderW3CASVRenesisEventDefaultImpl.SINGLETON;
    }
