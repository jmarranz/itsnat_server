...
public class JSRenderW3CMutationEventImpl extends JSRenderW3CEventImpl
{
    ...
    public JSRenderW3CMutationEventImpl()
    {
    }
    ...
    public String getInitEvent(Event evt,String evtVarName,ClientDocumentStfulDelegateWebImpl clientDoc)
    {
        ...
        return evtVarName + ".initMutationEvent("
                + "\"" + mutEvt.getType() + "\","
                + mutEvt.getBubbles() + ","
                + mutEvt.getCancelable() + ","
                + clientDoc.getNodeReference(mutEvt.getRelatedNode(),true,false) + ","  // No estoy seguro si el relatedNode puede ser null pero evitamos lanzar error en ese caso (la opción más prudente)
                + toTransportableStringLiteral(mutEvt.getPrevValue(),clientDoc.getBrowserWeb()) + ","
                + toTransportableStringLiteral(mutEvt.getNewValue(),clientDoc.getBrowserWeb()) + ","
                + "\"" + mutEvt.getAttrName() + "\","
                + mutEvt.getAttrChange()
                + ");\n";
    }
}
